...
def parse_args():
	...
	parser.add_argument("--dataset_name", type=str, default="Twitter16", choices=["Twitter15", "Twitter16"])
	parser.add_argument("--data_root", type=str, default="/mnt/hdd1/projects/BiGCN/dataset")
...
if __name__ == "__main__":
	...
	for iter in range(args.iterations):
		...
		fold4_x_test, fold4_x_train = load5foldData(args, args.dataset_name)
		treeDic = loadTree(args, args.dataset_name)
		train_losses, val_losses, train_accs, val_accs0, accs0, F1_0, F2_0, F3_0, F4_0 = train_GCN(args, treeDic,
																								   fold0_x_test,
																								   fold0_x_train,
																								   ...
																								   args.dataset_name,
																								   iter)
		train_losses, val_losses, train_accs, val_accs1, accs1, F1_1, F2_1, F3_1, F4_1 = train_GCN(args, treeDic,
																								   fold1_x_test,
																								   fold1_x_train,
																								   args.TDdroprate, args.BUdroprate, 
																								   args.lr, args.weight_decay,
																								   ...
																								   args.dataset_name,
																								   iter)
		train_losses, val_losses, train_accs, val_accs2, accs2, F1_2, F2_2, F3_2, F4_2 = train_GCN(args, treeDic,
																								   fold2_x_test,
																								   fold2_x_train,
																								   args.TDdroprate, args.BUdroprate, 
																								   args.lr, args.weight_decay,
																								   ...
																								   args.dataset_name,
																								   iter)
		train_losses, val_losses, train_accs, val_accs3, accs3, F1_3, F2_3, F3_3, F4_3 = train_GCN(args, treeDic,
																								   fold3_x_test,
																								   fold3_x_train,
																								   args.TDdroprate, args.BUdroprate, 
																								   args.lr, args.weight_decay,
																								   ...
																								   args.dataset_name,
																								   iter)
		train_losses, val_losses, train_accs, val_accs4, accs4, F1_4, F2_4, F3_4, F4_4 = train_GCN(args, treeDic,
																								   fold4_x_test,
																								   fold4_x_train,
																								   args.TDdroprate, args.BUdroprate, 
																								   args.lr, args.weight_decay,
	...
	if not os.path.isfile("{}/result.tsv".format(args.output_root)):
		with open("{}/result.tsv".format(args.output_root), "w") as fw:
			fw.write("{:10s}\t{:10s}\t{:10s}\t{:10s}\t{:10s}\t{:10s}\t{:10s}\n".format("Dataset", "Accuracy", "F1-Macro", "F1-NR", "F1-FR", "F1-TR", "F1-UR"))

	with open("{}/result.tsv".format(args.output_root), "a") as fw:
		fw.write("{:10s}\t{:<10.4f}\t{:<10.4f}\t{:<10.4f}\t{:<10.4f}\t{:<10.4f}\t{:<10.4f}\n".format(
				args.dataset_name, sum(test_accs) / args.iterations, 
				(sum(NR_F1) + sum(FR_F1) + sum(TR_F1) + sum(UR_F1)) / 4 * args.iterations, 
				sum(NR_F1) / args.iterations, 
				sum(FR_F1) / args.iterations, 
				sum(TR_F1) / args.iterations, 
				sum(UR_F1) / args.iterations
			)
		)
